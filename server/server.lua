...
AddEventHandler('esx:playerLoaded', function(src)
    ...
    if source == 0 or source == '' then
        MySQL.scalar('SELECT `fs-playtime` FROM `users` WHERE `identifier` = @identifier LIMIT 1', {
            ...
            local message = "**Player:** " .. GetPlayerName(src) .. " Has joined and their playtime has been loaded, their playtime is: " ..secconvert(playerData[src]['fs-playtime']) .. "."
            discordLog(message)
        end)
    end
end)
...
AddEventHandler('esx:playerDropped', function(src, reason)
    ...
    if source == 0 or source == '' then
        ...
        local message = "**player:** " .. GetPlayerName(src) .. " logged out and their playtime has been saved, Current playtime: " ..secconvert(playerData[src]['fs-playtime']) .. "."
        discordLog(message)
    end
end)

RegisterCommand(FS.command, function(source, args, rawCommand)
    TriggerClientEvent('chat:addMessage', source, { args = { 'Your total playtime is: ' ..secconvert(playerData[source]['fs-playtime']) .. ' !' }})
end)
